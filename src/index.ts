...
import { MagisterClient, ScheduleItem, StoredAppointment } from './magister.js';
...
function formatAppointments(items: StoredAppointment[]): string {
  if (items.length === 0) {
  ...
  return items
    .map((a) => {
      const start = new Date(a.startTime).toLocaleString('nl-NL', {
        dateStyle: 'short',
        timeStyle: 'short',
        timeZone: 'Europe/Amsterdam',
      });
      const end = new Date(a.endTime).toLocaleTimeString('nl-NL', {
        hour: '2-digit',
        minute: '2-digit',
        timeZone: 'Europe/Amsterdam',
      });
      const desc = a.description ? ` - ${a.description}` : '';
      return `${start} - ${end}: ${a.subject}${desc}`;
    })
    .join('\n');
}
...
server.setRequestHandler(ListToolsRequestSchema, async () => ({
  tools: [
    ...
    },
    {
      ...
      description:
        'Add a custom appointment to the schedule. Magister does not expose a public create-appointment API for student/parent accounts, so the appointment is stored locally and merged into the schedule. Note: this does NOT add the appointment to Magister itself.',
      inputSchema: {
        type: 'object',
        properties: {
          subject: { type: 'string', description: 'Subject or title of the appointment' },
          start: {
            type: 'string',
            description: 'Start time as ISO 8601 (YYYY-MM-DDTHH:mm) or YYYY-MM-DD HH:mm',
          },
          end: {
            type: 'string',
            description: 'End time as ISO 8601 (YYYY-MM-DDTHH:mm) or YYYY-MM-DD HH:mm',
          },
    ...
    },
    {
      ...
      description: 'List all locally-added custom appointments',
      inputSchema: {
        type: 'object',
        properties: {},
      },
  ...
  try {
    ...
    switch (name) {
      ...
      case 'add_appointment': {
        ...
        const record = await magister.addAppointment(input);
        return {
          content: [
            {
              type: 'text',
              text: `Appointment added: ${record.subject} (${record.startTime} - ${record.endTime})`,
            },
          ],
        };
      }
      ...
      case 'list_appointments': {
        const appointments = await magister.listAppointments();
        return {
          content: [
            {
              type: 'text',
